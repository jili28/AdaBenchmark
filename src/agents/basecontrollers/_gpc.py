...
from src.agents.basecontrollers._lqr import LQR
from src.agents.basecontrollers.agent import DAC
...
class GPC(DAC):
    def __init__(
        ...
        def policy_loss(M, w):
            ...
            def action(state, h):
                """Action function"""
                return jnp.clip(-self.K @ state + jnp.tensordot(
                    M, jax.lax.dynamic_slice_in_dim(w, h, H),
                    axes=([0, 2], [0, 1])), -2, 2)
    ...
    def update(self, state: jnp.ndarray, u: jnp.ndarray, A: jnp.ndarray, B: jnp.ndarray) -> None:
        """
        ...
        Returns:
            None
        """
        # added updating of the system matrices as it is highly needed
        self.A = A
        self.B = B
    ...
    def get_action(self, state: jnp.ndarray) -> jnp.ndarray:
        ...
        """
        a = -self.K @ state
        b = jnp.tensordot(self.M, self.last_h_noises(), axes=([0, 2], [0, 1]))
